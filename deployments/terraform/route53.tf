...
resource "aws_route53_record" "hero_ticket_client_record" {
  zone_id = aws_route53_zone.hero_ticket_zone.zone_id
  name    = aws_route53_zone.hero_ticket_zone.name
  type    = "A"

  alias {
    name                   = aws_cloudfront_distribution.hero_ticket_client_distribution.domain_name
    zone_id                = aws_cloudfront_distribution.hero_ticket_client_distribution.hosted_zone_id
